...
// Home v5 — "live post": estado presente arriba (AHORA), el objeto icónico (hemiciclo),
// la crónica cronológica, y el análisis (cinco lecturas) después. El lector casual sale
// informado con el hero + AHORA; el lector intenso baja.
  ...
  return (
    <div className="dt-view">
      ...
      <div style={{ maxWidth: "1180px", margin: "0 auto", padding: "48px 28px 8px" }}>
        ...
        <p style={{ fontSize: "18px", lineHeight: 1.5, color: "#57534E", maxWidth: "720px", margin: "16px 0 0" }}>Las 257 bancas reales, coloreadas por su acompañamiento al oficialismo en las {V.nVotsWord} votaciones decisivas del período. No es color de partido: es comportamiento registrado, con fuentes citadas.</p>
      </div>

      {/* ④ AHORA — estado del recinto */}
      <div style={{ maxWidth: "1180px", margin: "0 auto", padding: "24px 28px 0" }}>
        <AhoraCard V={V} />
      </div>

      {/* ⑤ FILTROS globales (período · bloque · tipo de item) — viven en la URL */}
      <div style={{ maxWidth: "1180px", margin: "0 auto", padding: "18px 28px 0" }}>
        <FilterBar V={V} />
      </div>

      {/* ⑥ HEMICICLO (intacto — es la identidad del sitio) + composición fusionada */}
      ...
      <div style={{ maxWidth: "1180px", margin: "0 auto", padding: "14px 28px 0" }}>
        <div style={{ position: "relative", background: "#FFFFFF", border: "1px solid #E7E3DB", borderRadius: "16px", padding: "26px 30px 22px", boxShadow: "0 1px 2px rgba(28,26,23,0.03),0 14px 38px -28px rgba(28,26,23,0.22)" }}>
          ...
          {/* composición por bloque — fusionada como franja al pie de la card */}
          <div style={{ marginTop: "16px", borderTop: "1px solid #F0EDE6", paddingTop: "14px" }}>
            ...
            <div style={{ display: "flex", height: "22px", borderRadius: "6px", overflow: "hidden", border: "1px solid #EFEBE3" }}>
              {V.blocComp.map((b: any, i: number) => (
                <div key={i} title={b.tip} style={{ width: b.w, background: b.chip, opacity: 0.92 }}></div>
              ))}
            </div>
            <div style={{ display: "flex", gap: "14px", flexWrap: "wrap", marginTop: "9px" }}>
              {V.blocComp.map((b: any, i: number) => (
      ...
      <div style={{ maxWidth: "1180px", margin: "0 auto", padding: "58px 28px 0" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: "16px", flexWrap: "wrap", borderBottom: "1px solid #E7E3DB", paddingBottom: "14px" }}>
          <div>
            <div style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "#B45309" }}>Cobertura en curso — actas reales y agenda oficial</div>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "18px", alignItems: "center" }}>
          <span style={{ fontSize: "10.5px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "#9A958A" }}>Períodos históricos</span>
      ...
      {/* ⑧ CINCO LECTURAS — el análisis */}
      <div style={{ maxWidth: "1180px", margin: "0 auto", padding: "58px 28px 0" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: "16px", flexWrap: "wrap", borderBottom: "1px solid #E7E3DB", paddingBottom: "14px" }}>
          <div>
            <div style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "#B45309" }}>Análisis — los mismos datos, cinco preguntas</div>
            <h2 style={{ fontFamily: "var(--font-serif), Georgia, serif", fontWeight: 600, fontSize: "30px", letterSpacing: "-0.015em", margin: "6px 0 0" }}>Cinco lecturas de la Cámara</h2>
          </div>
          <a href="#/indices" style={{ fontSize: "13.5px", fontWeight: 600, textDecoration: "none", color: "#1C1A17" }}>Metodología completa →</a>
        </div>
        <div style={{ marginTop: "20px" }}>
          <CardFrame
            kicker="La foto completa"
            title="Alineamiento por banca — distribución de la Cámara"
            chips={V.homeChartChips}
            meta="257 bancas"
            corte={V.corte}
          >
            <StripPlot data={V.stripHome} daltonico={V.daltonico} onOpen={V.stripOpen} />
          </CardFrame>
        </div>
        <div className="dt-g4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "14px", marginTop: "14px" }}>
          {[
            ["#/indices/disciplina", "Disciplina de bloque", "Qué tan seguido cada bloque vota en paquete — y cuándo se partió."],
            ["#/indices/poder", "Poder de bisagra", "Quién es decisivo para llegar a 129, más allá de su tamaño."],
            ["#/indices/rupturas", "Disidencias", "Las bancas que rompieron con su bloque, con registro documentado."],
            ["#/indices/territorio", "Territorio", "El mapa distrito por distrito y qué se renueva en 2027."],
          ].map(([href, titulo, texto], i) => (
            <a key={i} href={href} className="hov-row" style={{ display: "block", background: "#FFFFFF", border: "1px solid #E7E3DB", borderRadius: "14px", padding: "16px 18px", textDecoration: "none", color: "#1C1A17", transition: "background .15s" }}>
              <div style={{ fontSize: "14px", fontWeight: 600 }}>{titulo}</div>
              <div style={{ fontSize: "12.5px", color: "#78736A", lineHeight: 1.5, marginTop: "5px" }}>{texto}</div>
              <div style={{ fontSize: "12px", fontWeight: 600, color: "#B45309", marginTop: "9px" }}>Ver índice →</div>
            </a>
          ))}
        </div>
      </div>
      ...
      {/* ⑨ HERRAMIENTAS */}
      <div className="dt-g3" style={{ maxWidth: "1180px", margin: "0 auto", padding: "58px 28px 0", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "20px" }}>
        <div style={{ border: "1px solid #E7E3DB", borderRadius: "14px", padding: "18px 20px", background: "#FFFFFF" }}>
          <div style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "#B45309" }}>Comparación directa</div>
          <h2 style={{ fontFamily: "var(--font-serif), Georgia, serif", fontWeight: 600, fontSize: "22px", letterSpacing: "-0.015em", margin: "6px 0 0" }}>Comparar diputados</h2>
          <div style={{ display: "flex", gap: "10px", marginTop: "14px" }}>
            {V.compareTeaser.map((t: any, i: number) => (
              <div key={i} onClick={t.onClick} className="hov-row" style={{ flex: 1, border: "1px solid #E7E3DB", borderRadius: "12px", padding: "12px 8px", textAlign: "center", background: "#FFFFFF", cursor: "pointer" }}>
                <div className="dt-num" style={{ width: "40px", height: "40px", borderRadius: "50%", margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: 500, color: "#FFFFFF", background: `${t.swatch} center/cover`, backgroundImage: t.fotoCss }}>{t.initials}</div>
                <div style={{ fontSize: "11.5px", fontWeight: 600, marginTop: "8px", lineHeight: 1.2 }}>{t.nombre}</div>
                <div className="dt-num" style={{ fontSize: "19px", fontWeight: 500, marginTop: "4px" }}>{t.indice}</div>
              </div>
            ))}
          </div>
          <button onClick={V.goComparador} style={{ marginTop: "14px", width: "100%", background: "#1C1A17", color: "#FAFAF9", border: "none", borderRadius: "10px", padding: "11px", fontFamily: "inherit", fontSize: "13.5px", fontWeight: 600, cursor: "pointer" }}>Abrir comparador →</button>
        </div>
        <div style={{ border: "1px solid #E7E3DB", borderRadius: "14px", padding: "18px 20px", background: "#FFFFFF", display: "flex", flexDirection: "column" }}>
          <div style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "#B45309" }}>Mayorías posibles</div>
          <h2 style={{ fontFamily: "var(--font-serif), Georgia, serif", fontWeight: 600, fontSize: "22px", letterSpacing: "-0.015em", margin: "6px 0 0" }}>Simulador de votación</h2>
          <p style={{ fontSize: "13px", color: "#78736A", lineHeight: 1.55, margin: "12px 0 0", flex: 1 }}>Asigná posición a cada bloque y mirá si una ley llega a los 129 votos — con la composición real de la Cámara.</p>
          <button onClick={V.goSimulador} style={{ marginTop: "14px", width: "100%", background: "#FFFFFF", color: "#1C1A17", border: "1px solid #1C1A17", borderRadius: "10px", padding: "11px", fontFamily: "inherit", fontSize: "13.5px", fontWeight: 600, cursor: "pointer" }}>Abrir simulador →</button>
        </div>
        <div style={{ border: "1px solid #E7E3DB", borderRadius: "14px", padding: "18px 20px", background: "#FFFFFF", display: "flex", flexDirection: "column" }}>
          <div style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "#B45309" }}>Transparencia</div>
          <h2 style={{ fontFamily: "var(--font-serif), Georgia, serif", fontWeight: 600, fontSize: "22px", letterSpacing: "-0.015em", margin: "6px 0 0" }}>Dieta y patrimonio</h2>
          <p style={{ fontSize: "13px", color: "#78736A", lineHeight: 1.55, margin: "12px 0 0", flex: 1 }}>Cuánto cobra un diputado según la fuente oficial, y cómo consultar su declaración jurada. También: <a href="#/movimientos" style={{ fontWeight: 600 }}>bloques y movimientos</a>.</p>
          <button onClick={V.goPatrimonio} style={{ marginTop: "14px", width: "100%", background: "#FFFFFF", color: "#1C1A17", border: "1px solid #1C1A17", borderRadius: "10px", padding: "11px", fontFamily: "inherit", fontSize: "13.5px", fontWeight: 600, cursor: "pointer" }}>Ver patrimonio →</button>
        </div>
      </div>
